...
Let's start by creating a 6-fold kaleidoscope pattern. This demonstrates the core algorithm: mapping every pixel to a single "wedge" through angle folding.

.. code-block:: python
   :caption: simple_kaleidoscope.py - Create a 6-fold symmetric pattern
   :linenos:
   :emphasize-lines: 22-25,31-32
   ...
   # Step 7: Sample from source using mirrored coordinates
   ...
   # Step 8: Apply circular mask and save
   mask = radius <= center - 10
   canvas = np.where(mask[:, :, np.newaxis], canvas, 0)

   Image.fromarray(canvas, mode='RGB').save('simple_kaleidoscope.png')
...
.. figure:: kaleidoscope_variations.png
   ...
   Comparison of different fold counts: 4-fold (top-left), 6-fold (top-right), 8-fold (bottom-left), and 12-fold (bottom-right). Higher fold counts create more intricate, detailed patterns.
...
.. figure:: wedge_reflection.png
   ...
   The kaleidoscope construction process: (1) A single 60-degree wedge, (2) the wedge with its reflection creating a "kite" shape, (3) the complete 6-fold pattern from rotating the kite.
...
Run ``simple_kaleidoscope.py`` and observe the 6-fold symmetric pattern.
...
Modify ``simple_kaleidoscope.py`` to achieve these goals:
...
In approximately 18 minutes, you have learned the mathematical foundations of kaleidoscope effects and implemented them in NumPy.
