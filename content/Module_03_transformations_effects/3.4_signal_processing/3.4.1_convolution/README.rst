...
.. figure:: convolution_concept.png
   ...
   The four steps of 2D convolution.
...
Run :download:`kernel_effects.py <kernel_effects.py>` to see how different kernels transform the same image:
...
Starting with the blur kernel from :download:`simple_convolution.py <simple_convolution.py>`, modify the kernel values to achieve these effects:
...
.. dropdown:: Solution
   ...
   This kernel sums to 0 and computes the difference between the middle row and the rows above/below, highlighting horizontal edges [Sobel1968]_.
...
.. dropdown:: Solution
   ...
   This is the horizontal kernel transposed. It computes differences between the middle column and the columns on either side. Sobel (1968) and Prewitt (1970) independently developed similar directional gradient operators [Sobel1968]_ [Prewitt1970]_.
...
Implement your own convolution function from scratch! Complete the ``apply_convolution()`` function in the starter code to process the Brandenburg Gate image [BrandenburgGateImage]_.

:download:`Download Brandenburg Gate image <../../../../../_static/images/bbtor.jpg>`
...
**Starter code:** :download:`Download convolution_starter.py <convolution_starter.py>`
...
.. code-block:: python
   ...
   # Load the Brandenburg Gate image
   ...
   def apply_convolution(image, kernel):
       ...
       # Edge-replicate padding: copy border pixels outward so kernel always has valid neighbors [NumPyDocs]_
...
.. dropdown:: Complete Solution
   ...
   .. code-block:: python
      ...
      def apply_convolution(image, kernel):
          ...
          # Pad the image to handle borders [NumPyDocs]_
          padded = np.pad(image, pad, mode='edge')
